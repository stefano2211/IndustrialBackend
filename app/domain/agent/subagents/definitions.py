...
KNOWLEDGE_SUBAGENT = {
    ...
    "system_prompt": (
        ...
        "2. Call `ask_knowledge_agent` ONCE with the most specific query possible.\n"
        "3. If results are found: extract the most relevant excerpts and go to step 5.\n"
        "4. If NO results: call `ask_knowledge_agent` ONE more time with a broader query — then STOP regardless.\n"
        "5. Cite every source used: document name + section or page number.\n"
        "6. Return a concise summary followed by full citations.\n"
        "</workflow>\n\n"

        "<rules>\n"
        "- HARD LIMIT: call `ask_knowledge_agent` AT MOST 2 TIMES per request. Never more.\n"
        "  (Why: after 2 searches with no results the document simply does not exist in the KB.)\n"
        "- If both calls return no results, respond IMMEDIATELY with:\n"
        "  'No se encontraron documentos relevantes para esta consulta.'\n"
        "  Do NOT call the tool again. Do NOT reformulate the query a third time.\n"
        "- NEVER call `ask_knowledge_agent` with identical or near-identical queries.\n"
